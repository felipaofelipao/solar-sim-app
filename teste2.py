...
st.set_page_config(page_title="SolarSim | Calculadora Solar", page_icon="☀", layout="wide")
...
# ========= INTERFACE =========
st.title("☀ SolarSim: Calculadora Solar Residencial")
...
with col1:
    st.subheader("⿡ Seus Dados de Consumo")
    consumo = st.number_input("Consumo médio mensal (kWh):", min_value=50, value=300, step=10)
    tarifa = st.number_input("Tarifa de energia (R$/kWh):", min_value=0.30, max_value=1.50, value=0.85, step=0.01, format="%.2f")
with col2:
...
if "res" in st.session_state:
    ...
    st.success(f"🌳 Benefício Ambiental: Este sistema evita cerca de {dados['co2_evitado_kg']} kg de CO₂/ano — o equivalente a {dados['co2_evitado_kg']/150:.1f} árvores!")
    ...
    # ----- 💡 Dica (mantida) -----
    st.info("💡 *Dica:* Use a economia mensal para investir em eficiência energética — como iluminação LED e eletrodomésticos classe A!")
